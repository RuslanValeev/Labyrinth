    ...
    bool Collision(int dir)
    {
        for (int i = rect.top/23 ; i<(rect.top+rect.height)/23; i++)
            for (int j = rect.left/23; j<(rect.left+rect.width)/23; j++)
            {
                ...
                {
                    if ((dx>0) && (dir==0)) rect.left =  j * 23 -  rect.width;
                    if ((dx<0) && (dir==0)) rect.left =  j * 23 + 23;
                    if ((dy>0) && (dir==1))  { rect.top =   i * 23 -  rect.height;  dy=0;}
                    if ((dy<0) && (dir==1))  { rect.top = i * 23 + 23;   dy=0;}
                    return 0;
                }
                if (Map_1[i][j]=='2')
                {
...
private:
    ...
    void print_base ()
    {
        for(int i = 0; i < H; i++)
            for( int j = 0; j < W; j++)
            {
                if (this->BaseMap[i][j] == '1') rectangle.setFillColor(Color::Black);
                if (this->BaseMap[i][j] == '2') rectangle.setFillColor(Color::White);
                if (this->BaseMap[i][j] == '3') rectangle.setFillColor(Color::White);
                if (this->BaseMap[i][j] == '0') continue;
...
public:
    bool show(Clock clock, std::string Map[H])
    {
        ...
        while (window.isOpen() && key)
        {
            ...
            if (Keyboard::isKeyPressed(Keyboard::Left)) this->p.dx = -0.1;
            if (Keyboard::isKeyPressed(Keyboard::Right)) this->p.dx = 0.1;
            if (Keyboard::isKeyPressed(Keyboard::Up)) this->p.dy = -0.1;
            if (Keyboard::isKeyPressed(Keyboard::Down)) this->p.dy = 0.1;
    ...
    int main()
    {
        ...
        while (window.isOpen() && Menu)
        {
            ...
            if (menu1.Pos(950, 400)) {menu1.SetColor(Color::Yellow); MenuNum1 = 1;}
            if (menu2.Pos(950, 550)) {menu2.SetColor(Color::Yellow); MenuNum1 = 2;}
            if (menu3.Pos(950, 700)) {menu3.SetColor(Color::Yellow); MenuNum1 = 3;}
                ...
                switch (MenuNum1) {
                    case 1:
                    {
                        while (Menu1)
                        {
                            ...
                            if (level1.Pos(150, 200)) {level1.SetColor(Color::Yellow); MenuNum2 = 1;}
                            if (level2.Pos(700, 200)) {level2.SetColor(Color::Yellow); MenuNum2 = 2;}
                            if (level3.Pos(1250, 200)) {level3.SetColor(Color::Yellow); MenuNum2 = 3;}
                            if (level4.Pos(1800, 200)) {level4.SetColor(Color::Yellow); MenuNum2 = 4;}
